...
- **Time Complexity (Thời gian)**: `O(n)` — Thuật toán thực hiện đúng `n - 1` phép cộng tuần tự để tính `F(n)`, loại bỏ hoàn toàn các phép tính trùng lặp của đệ quy thông thường `O(2^n)`.
- **Space Complexity (Không gian bộ nhớ)**: `O(1)` — Tối ưu bộ nhớ tối đa bằng cách chỉ sử dụng 3 biến phụ trợ (`prev2`, `prev1`, `current`). So với phương pháp Memoization dùng mảng/Map (`O(n)` không gian), phương pháp Bottom-Up này tiết kiệm 100% dung lượng RAM phụ trợ.
